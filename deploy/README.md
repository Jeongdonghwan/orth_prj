...
- 배포 경로 `/var/www/ortho`, 서비스 `ortho.service`, 포트 **8031** (다른 포트를 쓰면 `ortho.service`·`nginx.conf`의 8031 을 함께 바꾼다)
- 사용자는 MobaXterm 으로 root 접속. 아래 블록을 순서대로 붙여넣는다.

## 1. 포트 확인 + 코드 + venv + .env + systemd

```bash
ss -tlnp | grep ':8031 ' && echo "!! 8031 사용중" || echo "8031 비어있음 OK"

cd /var/www && git clone <REPO_URL> ortho && cd ortho
...
sed -i "s|^SITE_URL=.*|SITE_URL=http://211.45.175.195:8031|" .env
...
curl -s http://127.0.0.1:8031/ | grep -o "<title>[^<]*</title>"
curl -s -o /dev/null -w "admin:%{http_code}\n" http://127.0.0.1:8031/admin/inquiries
curl -s http://127.0.0.1:8031/robots.txt
ufw status | head -1
```

기대 출력: `active (running)` / `<title>[병원명]정형외과의원 …</title>` / `admin:401` / robots 내용.
`ufw` 가 active 면 `ufw allow 8031/tcp`. 실패 시 `journalctl -u ortho -n 30 --no-pager`.
...
ufw delete allow 8031/tcp   # 프록시 뒤로 숨김
