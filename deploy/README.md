...
- 배포 경로 `/var/www/ortho`, 서비스 `ortho.service`, 포트 **8037** (다른 포트를 쓰면 `ortho.service`·`nginx.conf`의 8037 을 함께 바꾼다)
- 사용자는 MobaXterm 으로 root 접속. 아래 블록을 순서대로 붙여넣는다.

## 1. 포트 확인 + 코드 + venv + .env + systemd

```bash
ss -tlnp | grep ':8037 ' && echo "!! 8037 사용중" || echo "8037 비어있음 OK"
...
sed -i "s|^SITE_URL=.*|SITE_URL=http://211.45.175.195:8037|" .env
...
curl -s http://127.0.0.1:8037/ | grep -o "<title>[^<]*</title>"
curl -s -o /dev/null -w "admin:%{http_code}\n" http://127.0.0.1:8037/admin/inquiries
curl -s http://127.0.0.1:8037/robots.txt
ufw status | head -1
```
...
ufw delete allow 8037/tcp   # 프록시 뒤로 숨김
